...
data_try <- data_f[which(data_f$relfml==1),]
...
data_check <- data_check[which(data_f$relfml==1),]
...
#==== Create "Age at that preg" variable===
...
#==== Create "First ever depression at age t-2?"===
first_dep <- numeric()
for(j in 1:length(unique(melt_data_store$id)))
{
  pp <- nrow(melt_data_store[melt_data_store$id==unique(melt_data_store$id)[j],])
...
trt_mean <- apply(trt_var, 2, mean)
cont_mean <- apply(cont_var, 2, mean)
trt_variance <- apply(trt_var,2,var)
cont_variance <- apply(cont_var,2,var)
