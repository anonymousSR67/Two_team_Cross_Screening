...
#The above p-value is greater than 0.025. So the process has to stop here (see the analysis plan prepared by team B). 
#However, we still go ahead and compute the remaining p-values. In the right panel of Figure 6.1, we report
#all the p-values - though the process stops in the very first step itself. 
...
tt <- senmw(temp11-temp2,gamma = 1, method = "t")
pv3 <- tt$pval
...
testing <- senU(temp,gamma=1.2,m=8,m1=6,m2=7)
pv5 <- testing$pval


#============ Step 6: effect modification by age for Gamma =1.2====
tt <- senmw(temp11-temp2,gamma = 1.2, method = "t")
pv6 <- tt$pval
